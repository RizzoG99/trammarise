...
module.exports = __toCommonJS(summarize_exports);
...
// api/_utils/pdf-extractor.ts
var pdfjsLib = __toESM(require("pdfjs-dist/legacy/build/pdf.mjs"));
...
// api/_utils/text-chunker.ts
function chunkText(text, targetSize = 1e4) {
...
function shouldUseMapReduce(text, threshold = 15e3) {
...
var PDF_SIGNATURE = [37, 80, 68, 70];
function validatePdfFile(buffer, maxSizeBytes = 10 * 1024 * 1024) {
...
var { MAX_TRANSCRIPT_LENGTH, MIN_TRANSCRIPT_LENGTH, MIN_API_KEY_LENGTH, MAX_API_KEY_LENGTH } = API_VALIDATION;
...
`;
    ...
    if (contentType && !CONTENT_TYPE_VALUES.includes(contentType)) {
      return res.status(400).json({
        error: `Invalid content type. Must be one of: ${CONTENT_TYPE_VALUES.join(", ")}`
      });
    }
    if (!provider) {
    ...
    if (userProvidedKey) {
      if (userProvidedKey.length < MIN_API_KEY_LENGTH || userProvidedKey.length > MAX_API_KEY_LENGTH) {
      ...
      const { data: subscription, error: subError } = await supabaseAdmin.from("subscriptions").select("tier").eq("user_id", userId).single();
      if (subError || !subscription || subscription.tier === "free") {
        return res.status(403).json({
          error: "API key required",
          message: "Free tier users must provide their own API key. Upgrade to Pro or Team to use platform infrastructure.",
          upgradeUrl: "/pricing"
        });
      }
    ...
    if (shouldUseMapReduce(transcript)) {
      ...
      const chunks = chunkText(transcript, 1e4);
      ...
      const partialSummaries = await Promise.all(
        chunks.map((chunk, index) => {
          ...
          return aiProvider.summarize({
            transcript: chunk,
            contentType: "other",
            // Use generic for partial summaries
            apiKey: finalApiKey,
            model: actualModel,
            language,
            context: {
      ...
      const combinedPartials = partialSummaries.map((s, i) => `**Part ${i + 1}:**
${s}`).join("\n\n");
    ...
    await trackUsage(userId, "summarization", 60, mode);
