...
export function CollapsibleConfigPanel({ onConfigChange }: CollapsibleConfigPanelProps) {
  ...
  return (
    <GlassCard variant="light" className="mt-6">
      ...
      >
        <div>
          <Heading level="h3" className="mb-1">Configuration</Heading>
          <p className="text-sm text-text-secondary">
            {isExpanded ? 'Customize processing settings' : `${mode === 'balanced' ? 'Balanced' : 'Quality'} mode • ~${estimatedCredits} credits`}
          </p>
        </div>
        {isExpanded ? (
